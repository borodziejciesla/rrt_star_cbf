...
for index = 1:15
    objects_list{index} = struct('c', rand(2, 1), 'r', 0.08);
end
...
for i = 1:N
    rng(i);
    [path, path_length, mean_h_path, rejection_ratio, iterations_number] = SafeCBFRRTStar(start, goal, occupancy_map, h, dhdx, dhdy, params);
    figure(without_cbf);
